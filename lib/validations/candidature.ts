...
export const candidatureSchema = z.object({
  ...
  motivation: z.string().min(500, 'Le texte de motivation doit contenir au moins 500 caractères'),
  cv: z.instanceof(File, { message: 'Le CV est requis' }).refine(
  ...
  coverLetter: z.instanceof(File).optional().refine(
    (file) => !file || file.size <= 5 * 1024 * 1024,
    'Le fichier ne doit pas dépasser 5 Mo'
  ),
  otherDocument: z.instanceof(File).optional().refine(
    (file) => !file || file.size <= 5 * 1024 * 1024,
    'Le fichier ne doit pas dépasser 5 Mo'
  ),
  acceptPrivacy: z.boolean().refine((val) => val === true, 'Vous devez accepter le traitement de vos données'),
