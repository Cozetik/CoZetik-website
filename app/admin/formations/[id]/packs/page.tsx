import AddPackDialog from "@/components/admin/formations/packs/add-pack-dialog";
import PacksTable from "@/components/admin/formations/packs/packs-table";
...
export default async function FormationPacksPage({
  params,
}: {
  ...
  const nextOrder =
    formation.packs.length > 0
      ? Math.max(...formation.packs.map((p) => p.order)) + 1
      : 0;
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          <div className="space-y-3">
            ...
            <p className="text-gray-600 max-w-2xl">
              Gérez les différentes offres (Découverte, Premium, Expert) proposées pour cette formation
            </p>
          </div>

          <AddPackDialog formationId={id} nextOrder={nextOrder} />
        </div>
      </div>
        ...
        <Card className="rounded-2xl border-2 border-dashed border-gray-200">
          <CardContent className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-gray-600 mb-8 text-center max-w-md">
              Ajoutez vos premières offres tarifaires. En général : Découverte, Premium et Expert.
            </p>

            <AddPackDialog formationId={id} nextOrder={0} />
          </CardContent>
        </Card>
      ) : (
