...
async function uploadFileToCloudinary(file: File, folder: string = 'cozetik/candidatures'): Promise<string | null> {
  try {
    if (!file || file.size === 0) {
      return null;
    }
    ...
    if (
      ...
      console.warn('Cloudinary credentials are not configured, skipping file upload');
      return null;
    }
    ...
    if (file.size > maxSize) {
      console.warn(`File ${file.name} is too large (${file.size} bytes), skipping upload`);
      return null;
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validation basique
    if (!data.firstName || !data.lastName || !data.email || !data.phone || !data.formation) {
    ...
    if (data.cv && data.cv instanceof File) {
      uploadPromises.push(
        uploadFileToCloudinary(data.cv, 'cozetik/candidatures/cv').then(url => { cvUrl = url; })
      );
    }
    if (data.coverLetter && data.coverLetter instanceof File) {
      uploadPromises.push(
        uploadFileToCloudinary(data.coverLetter, 'cozetik/candidatures/lettres').then(url => { coverLetterUrl = url; })
      );
    }
    if (data.otherDocument && data.otherDocument instanceof File) {
      uploadPromises.push(
        uploadFileToCloudinary(data.otherDocument, 'cozetik/candidatures/documents').then(url => { otherDocumentUrl = url; })
      );
    }

    // Attendre que tous les uploads soient terminés
    await Promise.all(uploadPromises);
    console.log('📎 Uploads terminés:', { cvUrl, coverLetterUrl, otherDocumentUrl });
