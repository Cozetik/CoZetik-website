...
interface ViewInscriptionDialogProps {
  inscription: FormationInscription
}

export function ViewInscriptionDialog({
  inscription,
}: ViewInscriptionDialogProps) {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Demande reçue le{' '}
            {format(new Date(inscription.createdAt), 'PPP à HH:mm', {
              locale: fr,
            })}
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 pt-4">
          <div>
            ...
            <div className="mt-1">{getStatusBadge(inscription.status)}</div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="mt-1 text-base">{inscription.name}</p>
            </div>
