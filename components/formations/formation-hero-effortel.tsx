...
export default function FormationHeroEffortel({
  ...
  useEffect(() => {
    if (typeof window === 'undefined' || !containerRef.current || !carouselRef.current || steps.length <= 1) return
  ...
  return (
    <section ref={containerRef} className="relative bg-cozetik-black overflow-hidden">
        ...
        <div className="grid lg:grid-cols-[45%_55%] gap-12 items-center">
          ...
          >
            ...
            <Button
              ...
              className="bg-cozetik-green hover:bg-[#4A7A4A] text-white font-semibold text-lg px-10 py-6 rounded-none transition-all duration-300 hover:scale-105 mb-12"
            >
              ...
              >
                ...
                {steps.map((step, index) => {
                  ...
                  return (
                    ...
                    >
                      {/* Card */}
                      <div className="bg-white/5 backdrop-blur-sm border border-white/10 p-10 pt-12 rounded-2xl h-[560px] flex flex-col relative overflow-hidden">

                        {/* Background Gradient */}
                        <div className="absolute top-0 right-0 w-32 h-32 bg-cozetik-green/10 blur-3xl rounded-full" />

                        {/* Icon avec numéro */}
                        <div className="relative w-16 h-16 rounded-xl bg-gradient-to-br from-cozetik-green/30 to-cozetik-green/10 flex items-center justify-center mb-6">
