...
interface FormationHeroScrollProps {
  formation: {
    title: string
    description: string
    category: {
...
export default function FormationHeroScroll({
  formation,
  steps
}: FormationHeroScrollProps) {
  const scrollToForm = () => {
  ...
  useEffect(() => {
    if (typeof window === 'undefined' || !sectionRef.current || !carouselRef.current || steps.length === 0) return

    const section = sectionRef.current
    const carousel = carouselRef.current

    // Calculer la largeur totale de scroll (nombre de cards × largeur card)
    const cardWidth = 400 // Largeur d'une card en pixels
    const totalWidth = steps.length * cardWidth
    ...
    const tl = gsap.timeline({
      scrollTrigger: {
        trigger: section,
        start: 'top top',
        end: `+=${totalWidth * 2}`, // Plus de scroll = plus fluide
        scrub: 1, // Smooth scrub
        pin: true, // Pin la section pendant le scroll
        onUpdate: (self) => {
          // Mettre à jour l'index actif selon la progression
    ...
    // Animer le carousel horizontalement
    tl.to(carousel, {
      x: -(totalWidth - cardWidth), // Scroll jusqu'à la dernière card
      ease: 'none'
    })
  ...
  return (
    <section ref={sectionRef} className="relative bg-cozetik-black overflow-hidden min-h-screen">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <Button
              ...
              className="bg-cozetik-green hover:bg-cozetik-green-dark text-white font-semibold text-lg px-10 py-6 rounded-none transition-all duration-300 hover:scale-105"
            >
                  ...
                  {steps.map((step, index) => {
                    ...
                    return (
                      <div
                        key={step.id}
                        className={cn(
                          "flex-shrink-0 w-[400px] transition-all duration-500",
